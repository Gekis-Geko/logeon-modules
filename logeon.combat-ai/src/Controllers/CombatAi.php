...
namespace Modules\Logeon\CombatAi\Controllers;
...
use Modules\Logeon\CombatAi\Services\CombatAiService;

class CombatAi
{
    /** @var LoggerInterface|null */
    private $logger = null;
    /** @var CombatAiService|null */
    ...
    public function setService(CombatAiService $service = null)
    {
    ...
    private function service(): CombatAiService
    {
        if ($this->service instanceof CombatAiService) {
            return $this->service;
        }

        $this->service = new CombatAiService();
        return $this->service;
    }
    ...
    public function adminProfileSave($echo = true)
    {
        $this->trace('Richiamato il metodo: ' . __METHOD__);
        $userId = $this->requireAdmin();
        $response = [
            'dataset' => ['id' => $this->service()->saveProfile($this->requestDataObject(false), $userId)],
            'success' => true,
        ];
    ...
    public function adminProfileDelete($echo = true)
    {
        ...
        $this->service()->deleteProfile((int) ($data->id ?? 0));
    ...
    public function declareSuggestedAction($echo = true)
    {
        $this->trace('Richiamato il metodo: ' . __METHOD__);
        $session = $this->requireUserCharacter(true);
        $response = [
            'dataset' => $this->service()->declareSuggestedAction(
